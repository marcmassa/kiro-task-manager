...
describe("sddKanban — effectiveColumn", () => {
  ...
  test("P4: effectiveColumn is total for any valid input", () => {
    fc.assert(
      fc.property(taskStatusArb, agentStateArb, sddPhaseNullArb, (status, state, sdd_phase) => {
        const col = effectiveColumn({ status }, { state, sdd_phase });
  ...
  test("sdd_phase=requirements → requirements column", () => {
    expect(
      effectiveColumn({ status: "todo" }, { state: "agent_working", sdd_phase: "requirements" }),
    ).toBe("requirements");
  });

  test("sdd_phase=design → design column", () => {
    expect(
      effectiveColumn({ status: "todo" }, { state: "agent_working", sdd_phase: "design" }),
    ).toBe("design");
  });

  test("sdd_phase=tasks → tasks column", () => {
    expect(
      effectiveColumn({ status: "in_progress" }, { state: "agent_working", sdd_phase: "tasks" }),
    ).toBe("tasks");
  });

  test("sdd_phase=execution → in_progress column", () => {
    expect(
      effectiveColumn({ status: "todo" }, { state: "agent_working", sdd_phase: "execution" }),
    ).toBe("in_progress");
  });

  // Unit: done execution falls back to task.status
  test("sdd_phase set but state=done → falls back to task.status", () => {
    expect(effectiveColumn({ status: "todo" }, { state: "done", sdd_phase: "requirements" })).toBe(
      "todo",
    );
    expect(effectiveColumn({ status: "done" }, { state: "done", sdd_phase: "design" })).toBe(
      "done",
    );
  });
  ...
  test("sdd_phase=null → todo", () => {
    expect(effectiveColumn({ status: "todo" }, { state: "assigned", sdd_phase: null })).toBe(
      "todo",
    );
  });
  ...
  test("sdd_phase=null → in_progress", () => {
    expect(
      effectiveColumn({ status: "in_progress" }, { state: "agent_working", sdd_phase: null }),
    ).toBe("in_progress");
  });

  test("sdd_phase=null → done", () => {
    expect(effectiveColumn({ status: "done" }, { state: "done", sdd_phase: null })).toBe("done");
  });

  // Unit: null execution → task.status
  test("no execution → task.status", () => {
    expect(effectiveColumn({ status: "todo" }, null)).toBe("todo");
    expect(effectiveColumn({ status: "done" }, null)).toBe("done");
  });
});
