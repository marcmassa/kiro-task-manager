...
export function HomePage({
  ...
  function workspaceStatusLabel(status?: string): string {
    switch (status) {
      case "connected":
        return i18n.t("repo.statusConnected");
      case "cloning":
        return i18n.t("repo.statusCloning");
      case "error":
        return i18n.t("repo.statusError");
      case "disconnected":
        return i18n.t("repo.statusDisconnected");
      default:
        return i18n.t("repo.statusNotConfigured");
    }
  }
    ...
    <div className="flex-1 flex flex-col min-h-screen">
      <PageHeader
        ...
        subtitle={t("home.welcomeSubtitle", {
          name: activeWorkspace?.name ?? t("home.defaultWorkspace"),
        })}
      ...
      <main className="flex-1 px-8 py-6 space-y-8">
        ...
        <div className="home-card hero-gradient flex items-center gap-6 md:flex-row flex-col">
          ...
          <div className="text-center md:text-left">
            <h2 className="text-lg font-semibold text-white">{t("home.kiroReady")}</h2>
            <p className="text-sm text-muted-400 mt-1">{t("home.kiroSubtitle")}</p>
          </div>
        </div>
        ...
        <div className="home-card flex items-center justify-between gap-4 flex-wrap">
          <div className="flex items-center gap-3 min-w-0">
            ...
            <div className="min-w-0">
              <p className="text-xs text-muted-400">{t("home.activeWorkspace")}</p>
          ...
          <div className="text-right">
            ...
            <p className="text-xs text-muted-500 mt-1">
              {activeWorkspace?.repoCurrentBranch
                ? t("home.branchLabel", { branch: activeWorkspace.repoCurrentBranch })
                : t("home.noBranch")}
            </p>
          </div>
        </div>
          ...
          <div className="home-stat-card">
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className="text-xs text-muted-400">{t("home.totalTasks")}</p>
              </div>
            </div>
          </div>
          ...
          <div className="home-stat-card">
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className="text-xs text-muted-400">{t("home.inProgress")}</p>
              </div>
            </div>
          </div>
          ...
          <div className="home-stat-card">
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className="text-xs text-muted-400">{t("home.completedPct")}</p>
              </div>
            </div>
          </div>
          ...
          <div className="home-stat-card">
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className="text-xs text-muted-400">{t("home.overdue")}</p>
              </div>
            </div>
          </div>
        </div>
        ...
        <div className="home-card">
          <div className="flex items-center justify-between mb-4">
            <h2 className="text-sm font-semibold text-gray-200">{t("home.progress")}</h2>
            <span className="text-xs text-muted-400">
              {t("home.pctCompleted", { pct: completionRate })}
            </span>
          </div>
          ...
          <div
            ...
            style={{ gridTemplateColumns: `repeat(${3 + customColumns.length}, minmax(0,1fr))` }}
          >
            {[
              { label: t("kanban.col_todo"), count: todoCount, dot: "bg-accent" },
              ...customColumns.map((c) => ({
                label: c.label,
                count: columnCounts.get(c.id) ?? 0,
                dot: columnColorTokens(c.color).dot,
              })),
              { label: t("kanban.col_inProgress"), count: inProgressCount, dot: "bg-warning" },
              { label: t("kanban.col_done"), count: doneCount, dot: "bg-success" },
            ].map((col) => (
        ...
        {customColumns.length > 0 && (
          <div className="home-card">
            <div className="flex items-center justify-between mb-4">
              <h2 className="text-sm font-semibold text-gray-200">{t("home.pipeline")}</h2>
              <button
                onClick={() => onNavigate("kanban")}
                className="text-xs text-accent-400 hover:text-accent-300 flex items-center gap-1 transition-colors"
              >
                {t("home.viewBoard")} <ArrowRightIcon size={12} />
              </button>
            </div>
            <div className="flex items-center gap-1 overflow-x-auto pb-1">
              {[
                { label: t("kanban.col_todo"), count: todoCount, dot: "bg-muted-500" },
                ...customColumns.map((c) => ({
                  label: c.label,
                  count: columnCounts.get(c.id) ?? 0,
                  dot: columnColorTokens(c.color).dot,
                ...
                <div key={col.label} className="flex items-center gap-1 shrink-0">
                  <div className="flex flex-col items-center gap-1 px-2">
                    ...
                    <div className="flex items-center gap-1">
                      ...
                      <span className="text-[10px] text-muted-400 whitespace-nowrap">
                        {col.label}
                      </span>
                    </div>
                  </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
          ...
          <div className="home-card">
            <div className="flex items-center justify-between mb-4">
              <h2 className="text-sm font-semibold text-gray-200">{t("home.recentTasks")}</h2>
              <button
                onClick={() => onNavigate("kanban")}
                className="text-xs text-accent-400 hover:text-accent-300 flex items-center gap-1 transition-colors"
              >
            ...
            <div className="space-y-2">
              {recentTasks.length === 0 ? (
                <div className="flex flex-col items-center gap-3 py-6">
                  ...
                  <p className="text-sm text-muted-400 text-center">{t("home.noTasks")}</p>
                </div>
              ) : (
          ...
          <div className="home-card">
            <div className="flex items-center justify-between mb-4">
              <h2 className="text-sm font-semibold text-gray-200">{t("home.overdueTasks")}</h2>
              {overdueTasks.length > 0 && (
                <span className="badge bg-danger/15 text-danger-400 text-[10px]">
                  {overdueTasks.length > 1
                    ? t("home.overduePendingMany", { count: overdueTasks.length })
                    : t("home.overduePendingOne", { count: overdueTasks.length })}
                </span>
              )}
            </div>
            <div className="space-y-2">
              {overdueTasks.length === 0 ? (
                <div className="text-center py-6">
                  ...
                  <p className="text-sm text-muted-400">{t("home.upToDate")}</p>
                </div>
              ) : (
