...
/**
 * WorkspaceBar — barra horizontal superior que muestra el workspace activo
 * con un selector dropdown. Visible en todas las páginas (Kanban, Stats, etc.).
 *
...
export function WorkspaceBar({
  ...
  }, [workspaces, active, activeWorkspaceId, onWorkspaceChange]);

  if (!active && workspaces.length === 0) return <div className="h-10" />;

  return (
    <div
      className="h-10 border-b border-white/5 bg-surface-500/50 flex items-center px-4 shrink-0"
      ref={dropdownRef}
    >
      ...
      {/* Dropdown */}
      {open && (
        <div className="absolute left-4 top-full mt-1 z-50 w-56 rounded-lg bg-surface-400 border border-white/10 shadow-xl overflow-hidden">
